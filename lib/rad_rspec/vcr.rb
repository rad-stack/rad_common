...
VCR.configure do |c|
  ...
  c.filter_sensitive_data('<OPEN_AI_API_KEY>') { RadConfig.open_ai_api_key! } if RadConfig.open_ai_api_key.present?
  ...
  c.filter_sensitive_data('<GCS_PROJECT_ID>') { RadConfig.gcs_project_id! }
