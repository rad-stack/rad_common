...
VCR.configure do |c|
  ...
  if RadicalConfig.secret_config_item(:twilio_alt_verify_sid).present?
    c.filter_sensitive_data('<TWILIO_ALT_VERIFY_SID>') { RadicalConfig.secret_config_item!(:twilio_alt_verify_sid) }
  end
