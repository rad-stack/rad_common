...
RSpec.describe RadReports::ValueFormatter, type: :service do
  describe '.format_record_value' do
    subject(:formatted_value) { described_class.format_record_value(record, select_clause, column_def) }
    ...
    context 'with nil value' do
      let(:record) { user }
      let(:select_clause) { 'nonexistent_column' }
  ...
  describe '.extract_value' do
    subject(:extracted_value) { described_class.extract_value(record, select_clause, column_def) }
    ...
    context 'with joined column notation' do
      ...
      it 'falls back to column name if underscore notation fails' do
        ...
        allow(fallback_record).to receive(:respond_to?).with(:[]).and_return(false)
        allow(fallback_record).to receive(:email).and_return('fallback@example.com')

        result = described_class.extract_value(fallback_record, 'owner.email', column_def)
    ...
    context 'with deeply nested joined column' do
      let(:select_clause) { 'owner.user_status.name' }
      let(:record) { double('record') }
