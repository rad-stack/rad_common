...
RSpec.describe RadReports::FilterRegistry, type: :service do
  ...
  describe '.filter_options' do
    subject(:options) { described_class.filter_options(class_name) }

    context 'with DateFilter' do
      let(:class_name) { 'RadSearch::DateFilter' }

      it 'returns configuration options array' do
        expect(options).to be_an(Array)
        expect(options.length).to be > 0
      end
    end

    context 'with filter without options' do
      let(:class_name) { 'RadSearch::LikeFilter' }
