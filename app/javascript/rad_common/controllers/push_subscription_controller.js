...
export default class extends Controller {
  ...
  async requestPermission() {
    ...
    if (permission === 'granted') {
      ...
      this.updateStatusDisplay(subscribed ? 'granted' : 'default');
    } else if (permission === 'denied') {
      this.updateStatusDisplay('denied');
    }
  }
    ...
    try {
      ...
      if (newSubscription) {
        try {
          ...
          console.error('[PushSubscription] Rollback unsubscribe failed:', rollbackError);
        }
      }
      return false;
    }
  ...
  async removeSubscriptionFromServer(endpoint) {
    ...
    await fetch('/push_subscriptions/unsubscribe', {
      method: 'DELETE',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
