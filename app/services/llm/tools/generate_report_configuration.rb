module LLM
  module Tools
    class GenerateReportConfiguration < Base
      def description
        'Creates a new custom report with the specified configuration. This should be called after gathering all necessary information about the report requirements. Returns success with report URL or validation errors.'
      end

      def required_params
        [:name, :model_name, :columns]
      end
      ...
      def call
        ...
        # Apply default formulas for appropriate column types
        config[:columns].each do |col|
          next if col['formula'].present?

          # Extract column info to determine type
          column_name = col['name']
          select_path = col['select']

          # Try to find the column type from the model
          if select_path && model_name
        ...
        # Build the configuration using the ConfigurationBuilder
        ...
        "ERROR: An error occurred while creating the report: #{e.message}. Please check the configuration and try again."
      end
    end
  end
end
