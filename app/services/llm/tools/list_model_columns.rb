  ...
  module Tools
    class ListModelColumns < Base
      HEADER_TEXT = <<~TEXT.freeze
        IMPORTANT: Use the exact 'select' value shown below when configuring columns.

      TEXT

      FOOTER_TEXT = <<~TEXT.freeze
        Column Configuration Format:
        When adding a column to your report, use this structure:
        {
          "name": "<column name from above>",
          "label": "<human readable label>",
          "select": "<exact select value from above>"
        }
      TEXT
