module Devise
  class DeviseTwilioVerifyController < DeviseController
    ...
    def POST_verify_twilio_verify
      ...
      verification_check = if @resource.twilio_totp_factor_sid.present?
                             TwilioVerifyService.verify_totp_token(@resource, params[:token])
                           else
                             TwilioVerifyService.verify_sms_token(@resource.mobile_phone, params[:token])
                           end
    ...
    def POST_verify_twilio_verify_installation
      ...
      resource.twilio_verify_enabled = verification_check.status == 'approved'

      if verification_check.status == 'approved' && resource.save
        remember_device(@resource.id) if params[:remember_device].to_i == 1
        record_twilio_verify_authentication
