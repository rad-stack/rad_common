module RadUsersHelper
  ...
  def add_user_client_action(user)
    ...
    link_to(icon(:plus, "Add #{AppInfo.new.client_model_label} to User"),
  ...
  def clients_to_add_to_user(user)
    policy_scope(AppInfo.new.client_model_class).active.where.not(id: user.clients.pluck(:id)).sorted
  end
